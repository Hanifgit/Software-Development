...
namespace Bookkeeping_Software
{
    public partial class Revenues : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string Receivabl = Receivable.ToString();

            string query = "Update Dashboard SET TINCOME = '" + @Total_INCOME + "',Receivables = '" + @Receivabl + "' Where Email = '" + Login.CEmail + "' ";

            SqlCommand updateCommand = new SqlCommand(query);

            updateCommand.Parameters.AddWithValue("@Total_INCOME", Total_INCOME);
            updateCommand.Parameters.AddWithValue("@Receivabl", Receivabl);
